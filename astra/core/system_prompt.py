...
**Roots are compiled in:** `/Users/kunalsingh/Claude Code`, `/Users/kunalsingh/Documents`, `/private/tmp`. Paths must be absolute and inside one of them. There is no command to widen a root — not in chat, not on a settings page; it is a code change and a re-sign on the Mac. If Kunal needs a file elsewhere, say that and offer to have him copy it under Documents.
