...
- Texting-voice corpus: Astra learns his WhatsApp/Instagram voice from the platforms' own EXPORTS (the only safe way — no API reads personal accounts). Flow when he wants this: WhatsApp → open chat → Export Chat (without media) → save the .txt on the Mac; Instagram → Settings → Download Your Information → messages JSON. Then he says "ingest <path>" → call `ingest_voice_export(channel, path, self_name)` (bridge must be up; ask for his display name as it appears IN the export if unknown). Re-runnable, deduped. `voice_profiles` shows everything learned so far.
